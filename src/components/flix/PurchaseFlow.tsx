...
const ORDER_WA = "201109664083";
const PAY_NUMBER = "01014956483";
const TELDA_ID = "@flix90";
const INSTAPAY_LINK = "https://ipn.eg/S/flixop90/instapay/2jsoLA";

const METHODS = [
  {
    id: "vodafone" as PayMethodId,
    icon: "📱",
    title: "فودافون كاش",
    detail: `حول على ${PAY_NUMBER}`,
    label: "Vodafone Cash",
  },
  {
    id: "telda" as PayMethodId,
    icon: "💳",
    title: "تيلدا",
    detail: TELDA_ID,
    label: "Telda",
...
export function PurchaseFlow({
  ...
  const canSubmit = Boolean(
    receipt && name.trim().length >= 2 && wa.trim().length >= 8,
  );
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          {/* Backdrop - separate from modal so clicks don't interfere with inputs */}
          <motion.div
            key="pf-backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <div
            ...
            dir="rtl"
          >
            <div className="min-h-full flex items-start sm:items-center justify-center p-3 sm:p-6 py-6">
              <motion.div
                key="pf-modal"
                ...
                className="relative w-full max-w-lg rounded-2xl border border-red-500/30 bg-[#0a0a0a] shadow-[0_0_60px_rgba(204,0,0,0.35)]"
              >
                {/* Close button */}
                <button
                  type="button"
                ...
                <div className="px-5 pt-5 pb-3 border-b border-white/5">
                  <div className="flex items-center gap-2 text-[11px] tracking-wider text-white/50">
                    {[1, 2, 3].map((s) => (
                ...
                <div className="p-5 sm:p-6">
                  ...
                  {step === 1 && (
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-2 text-white/80 text-sm flex-wrap">
                          {order.duration && (
                            <>
                              ...
                              <span>المدة: <span className="text-white">{order.duration}</span></span>
                              <span className="text-white/30">·</span>
                            </>
                          )}
                          {order.platform && (
                            <>
                              <span>المنصة: {order.platform}</span>
                              <span className="text-white/30">·</span>
                            </>
                          )}
                          ...
                          {METHODS.map((m) => {
                            ...
                            return (
                              <button
                                ...
                                className={`relative text-right rounded-xl border p-3 transition-all cursor-pointer ${
                                  active
                                    ? "border-red-500 bg-red-500/10 shadow-[0_0_18px_rgba(204,0,0,0.3)]"
                      ...
                      <button
                        ...
                        disabled={!method}
                        className="w-full py-3 px-5 rounded-lg bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-base flex items-center justify-center gap-2 transition"
                      >
                        التالي — تأكيد طريقة الدفع
                  ...
                  {step === 2 && selectedMethod && (
                    <div className="space-y-5">
                      <h3 className="text-2xl font-bold text-white">خطوات الدفع</h3>
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 space-y-3">
                        ...
                        <div className="text-sm text-white/70">
                          حول مبلغ{" "}
                          <span className="text-red-300 font-bold text-base">{order.price}</span>{" "}
                          جنيه على:
                        </div>

                        {method === "vodafone" && (
                          <div className="flex items-center justify-between gap-2 rounded-lg bg-black/40 border border-white/10 p-3">
                            <div dir="ltr" className="text-lg font-bold text-white tracking-wider">
                              📞 {PAY_NUMBER}
                            </div>
                      ...
                      <div className="rounded-xl border-2 border-red-500/60 bg-red-500/10 p-3.5 flex gap-2.5 text-sm text-red-100">
                        ...
                        <div>
                          <div className="font-bold tracking-wider mb-0.5">⚠️ مهم جداً</div>
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="w-full rounded-xl border-2 border-dashed border-red-500/40 bg-black/40 hover:border-red-500 hover:bg-red-500/5 transition p-5 flex flex-col items-center justify-center gap-2 text-white/70 cursor-pointer"
                        >
                          {receiptUrl ? (
                            <>
                              <img
                      ...
                      {/* Input Fields - NO motion wrappers, plain inputs for max compatibility */}
                      <div className="space-y-3">
                        <div>
                          <label
                            htmlFor="pf-name"
                            className="block text-xs text-white/60 mb-1.5"
                          >
                            الاسم
                          </label>
                          <input
                            ...
                            spellCheck={false}
                            className="w-full px-3 py-3 rounded-lg bg-[#111] border border-white/15 focus:border-red-500 focus:outline-none text-white placeholder:text-white/30 text-right text-base"
                            style={{ fontSize: "16px" }}
                          />
                        </div>
                        <div>
                      ...
                      <div className="flex flex-col-reverse sm:flex-row gap-2">
                        <button
                          type="button"
                          onClick={() => setStep(1)}
                          className="px-4 py-2.5 rounded-lg border border-white/15 text-white/70 hover:text-white hover:border-white/30 text-sm flex items-center justify-center gap-2 transition sm:w-auto"
                        >
                        ...
                        <button
                          ...
                          onClick={submitOrder}
                          disabled={!canSubmit}
                          className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3 rounded-lg font-bold bg-[#25D366] hover:bg-[#1ebe5b] text-black disabled:opacity-40 disabled:cursor-not-allowed transition text-base"
                        >
                      ...
                      {!canSubmit && (
                        <p className="text-[11px] text-red-400/70 text-center">
                          {!receipt && "• ارفع صورة الإيصال "}
                          {name.trim().length < 2 && "• أدخل اسمك "}
                          {wa.trim().length < 8 && "• أدخل رقم واتساب صحيح"}
                        </p>
                      )}
                  ...
                  {step === 3 && (
                    <div className="text-center space-y-5 py-2">
                      ...
                      <p className="text-white/70 text-sm leading-relaxed">
                        ...
                        <span dir="ltr" className="text-white font-bold">{wa}</span>
                        <br />
                      ...
                      <button
                        type="button"
                        onClick={onClose}
                        className="px-5 py-2.5 rounded-lg border border-white/15 text-white/70 hover:text-white hover:border-white/30 text-sm transition"
                      >
                        العودة للرئيسية
                      </button>
                    </div>
